...
# Adjust precision in fraction of mm
STEPPER_PULSES_PER_MM_X = 1000
STEPPER_PULSES_PER_MM_Y = 1000
STEPPER_PULSES_PER_MM_Z = 1000

class PulseGenerator(object):
    """ Stepper motors pulses generator.
        It generates movement for each timeframe for specified path as accelerated
...
class PulseGeneratorLinear(PulseGenerator):
    ...
    def _interpolation_function(self, ix, iy, iz):
        ...
        return self._direction, (t_x, t_y, t_z)
...
class PulseGeneratorCircular(PulseGenerator):
    ...
    def _interpolation_function(self, ix, iy, iz):
        ...
        if self._plane == PLANE_XY:
            ...
            dy = self._third_dir
        return (dx, dy, dz, self._e_dir), (tx, ty, tz)
